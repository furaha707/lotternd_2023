...
(function ($) {
  $.fn.navDrop = function (options) {
    ...
    var TYPE = validateCheck(settings.type), // 옵션 타입 ( cf. 하단 validateCheck 함수 참고 )
      ...
      $NAV_DROP = validateCheck($THIS.find('.nav .nav-list--depth2-wrap')), // .nav-list--depth2
    ...
    switch (TYPE) {
      ...
      var BTMLINK = $('.link-wrap'); 
      var DECOIMG = $('.nav-deco');
          ...
          $(this).on('mouseover focus', function () {
            if ($(this).next().length > 0) {
              if ( EFFECT === 'fade' ) {
                ...
                $(this).next().stop(false, true).show();

                if($(header).hasClass('off') === true){
                  $THIS.removeClass('gnb-hover');
                } 
              ...
              if ( EFFECT === 'fade' ) {
                ...
                $('.header-gnb').removeClass('gnb-hover');

              } else if ( EFFECT === 'slide' ) {
                $NAV_DROP.stop(false, true).slideUp(DELAY);
              }
            }
            if($(".gnb-hover").hasClass('gnb-hover') ){
              BTMLINK.show();
              DECOIMG.show();
            }
          });
    ...
    function backgroundActive(target){ 
      ...
      switch (TYPE) {
        ...
        case 'each': 
          ...
          $('.header-wrap .nav-list .depth-1').children('.link').each(function (index) {
            $(this).on('mouseover focus', function () {
              $NAV_DROP.each(function(){
                if ($(this).outerHeight() > maxHeight) {
                  maxHeight = $(this).outerHeight();
