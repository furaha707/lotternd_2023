...
var subNav = {
  ...
  depth_clone: function () {
    var $depth1Active = $('.nav').not('.nav-menu').find('.depth-1 > .link'); // 1dp 가져오기
    var $depth2Active = $('.nav').not('.nav-menu').find('.depth-1 > .link.on').next().find('.depth-2 > .link'); //활성화된 2depth
    var $depth3Active = $('.nav').not('.nav-menu').find('.depth-2 > .link.on').next().find('.depth-3 > .link'); //활성화된 3depth
...
var subNavScroll = {
  ...
  sticky_nav: function () {
    var $subNav = $('.sub-nav--bar'),
      $subNavWrap = $('.sub-nav--bar .sub-nav--box'),
 ...
 if ($(window).outerWidth() > 1200) {
            ...
            $('.history-tab-wrap').css('margin-top', '-101px');
 } else {
  $('.history-tab-wrap').css('margin-top', '-61px');
 }
          } else {
            ...
            $('.history-tab-wrap').css('margin-top', '0');
          }
        } else {
...
$(window).scroll(function (event) {
    if ($('body').hasClass('sub-body') && $(window).scrollTop() > $('.sub-visual').outerHeight()) {
...
$(document).ready(function(){
  ...
  //헤더메뉴 슬라이더
var noticeSwiper = new Swiper('.notice-bx .swiper-container', {
  loop: true,
  effect: 'fade',
  autoplay: {
    delay: 2000,
  ...
  pagination: {
    el: '.notice-bx .swiper-pagination',
    type: 'bullets',
    clickable: true,
  },
  navigation: {
    nextEl: '.notice-bx .swiper-button--next',
    prevEl: '.notice-bx .swiper-button--prev',
  },
  observer: true,	
  observeParents: true,	
});
...
//헤더 햄버거버튼
$('.total-item').on('click', function () {
  $('.header-gnb').toggleClass('off');
...
$(window).scroll(function(){
  if($('body').hasClass('main-body')){
    if($(window).innerWidth() < 1200 && $(window).scrollTop() > 50 ){
      $('header').addClass('fixed');
    } else if ($(window).innerWidth() < 1200 && $(window).scrollTop() < 50 ){
      $('header').removeClass('fixed');
    }
  } else if ($('body').hasClass('sub-body')){
...
// 푸터 패밀리사이트 스크립트
	$(document).ready(function () {
		$('.familyBtn').on('click', function (e) {
			e.preventDefault();
			$('.scrollbox').stop().slideToggle(250);
			$(this).toggleClass('on');
      ...
      $('body').on('scroll touchmove mousewheel', function (event) {
		...
		var familyLink = $('.scrollbox .inner .link');
		var familyGo = $('.familyGo');

		familyLink.each(function () {
			$(this).on('click', function (e) {
				e.preventDefault();
				$('.scrollbox').slideDown();
				var linkText = $(this)[0].innerText;
				var linkHref = $(this).attr('href');

				$('.familyBtn span').text(linkText);
				$('.familyBtn').attr('href', linkHref);
			});
		});

		familyGo.on('click', function () {
			var linkHref = $('.familyBtn').attr('href');
    ...
    $('body').on('click', function(e){
      ...
      var $familyInput = $tgPoint.parent().hasClass('familyBtn');
    
      if(!$familyInput){
        $('.scrollbox').slideUp(200);
